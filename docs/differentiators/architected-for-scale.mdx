...
Radius is architected for scale that matches the internet itself. 2.5 million TPS tested. 200ms settlement. Linear scalability — every addition of compute makes the system faster. This page shows how it works and proves it with live metrics.
...
```mermaid
flowchart TB
    subgraph Keyspace["Keyspace: 000000 → ffffff"]
        direction LR
    end
    ...
    subgraph S0["Shard 0<br/>000000-555554"]
        R0["3-node Raft cluster"]
    end

    subgraph S1["Shard 1<br/>555555-aaaaaa"]
        R1["3-node Raft cluster"]
    end

    subgraph S2["Shard 2<br/>aaaaaa-ffffff"]
        R2["3-node Raft cluster"]
    end
```
...
| Settlement latency | P99 under load | <200ms |
...
- [Quick Start](/build-deploy/quick-start) — Get from zero to first transaction
