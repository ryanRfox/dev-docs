...
State are `key : value` pairs representing data elements used with Radius Network, including smart contract code, their "data", and account balance. Radius distributes distinct state elements across multiple independent shard clusters:

<div style={{ display: 'flex', justifyContent: 'center', overflowX: 'auto', margin: '1.5rem 0' }}>

```mermaid
flowchart TB
    K["Radius Network Keyspace:<br/>0x00...00 → 0xff...ff"]

    K --> S0 & S1 & S2

    S0["Shard 0 Keyspace:<br/>0x00...00 → 0x55...54"]
    S1["Shard 1 Keyspace:<br/>0x55...55 → 0xaa...aa"]
    S2["Shard 2 Keyspace:<br/>0xaa...ab → 0xff...ff"]

    S0 --> CS0["Cluster 0"]

    subgraph C1["Cluster 1"]
        direction LR
        N1["Node 1.0"] ~~~ N11["Node 1.1"] ~~~ N12["Node 1.2"]
    end
    S1 --> C1

    S2 --> CS2["Cluster 2"]

    classDef shard fill:#fff3e0,stroke:#ff9800,stroke-width:2px
    classDef cluster fill:#e8f5e9,stroke:#4caf50,stroke-width:2px
    classDef node fill:#e3f2fd,stroke:#2196f3,stroke-width:2px

    class K,S0,S1,S2 shard
    class CS0,C1,CS2 cluster
    class N1,N11,N12 node
```
