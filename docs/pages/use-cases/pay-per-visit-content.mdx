...
export function ContentPaywall({
  ...
  return (
    <div className="paywall">
      ...
      >
        {isPaying || isConfirming ? 'Processing...' : `Unlock for ${amount} SBC`}
      </button>
...
```typescript
const contentTiers = {
  article: '0.05', // 0.05 SBC per article
  video: '0.25', // 0.25 SBC per video
  research: '1.00', // 1.00 SBC per research paper
  masterclass: '5.00', // 5.00 SBC per masterclass
};
...
- **Lower barrier than subscriptions** — Pay 0.10 SBC for one article instead of 15 USDC/month
...
With Radius, a creator earning 0.10 SBC per article only needs 100 readers per day to generate 1000 SBC/month. That's achievable for most niche content creators.
</Callout>
...
- **Bundle offers** — Sell article packs ("10 articles for 0.50 SBC") to increase AOV
...
<Callout type="info">
Radius transactions cost nearly nothing to process. A 0.10 SBC article payment costs less than 0.01 SBC in fees, so almost all revenue goes to the creator.
</Callout>
